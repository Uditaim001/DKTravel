...
const filePath = path.join(__dirname, '../data/hotel_requests.json');
const read = () => JSON.parse(fs.readFileSync(filePath));
const write = (data) => fs.writeFileSync(filePath, JSON.stringify(data, null, 2));

router.post('/', (req, res) => {
  const requests = read();
  const request = { id: uuidv4(), refNo: 'HT' + Date.now().toString().slice(-6), ...req.body, status: 'pending', createdAt: new Date().toISOString() };
  requests.push(request);
  write(requests);
  res.status(201).json({ message: 'Hotel booking request submitted! We will contact you within 24 hours.', refNo: request.refNo });
});
